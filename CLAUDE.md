...
## Cross-Origin CDN Compatibility

OWR serves builder assets from a CDN (`cdn.oldworldrankings.com`) via `PUBLIC_URL`. SVG `<use xlink:href>` elements **cannot reference cross-origin resources** — browsers block this regardless of CORS headers. The `Icon` component was converted from an SVG sprite (`symbol-defs.svg` with `<use>`) to inline SVG paths to avoid this. **Do not reintroduce SVG sprites with `<use>`** or the CDN setup will break icon rendering.
